...
def about(topic):
    ...
    def select(paragraph):
        ...
        new_split_list = split(paragraph)
        for i in range(0, len(topic)):
            for j in range(0, len(new_split_list)):
                if topic[i] == lower(new_split_list[j]):
                    return True
        return False
    return select
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    #             count = count + 1
    #     return (count / len(reference_words)) * 100
    # elif len(typed_words) > len(reference_words):
    #     for i in range(0, len(typed_words)):
    #         if typed_words[i] == reference_words[i]:
    #             count = count + 1
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    numchar = sum(len(i) for i in typed)
    return numchar / 5 * (60 / elapsed)
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    for k in valid_words:
        if typed_word == k:
            return typed_word
    possible = [valid_words[i] for i in range(len(valid_words)) if (diff_function(typed_word, valid_words[i], limit) <= limit)]
    numdifference = [diff_function(typed_word, possible[j], limit) for j in range(len(possible))]
    if possible == []:
        return typed_word
    else:
        min_value = min(numdifference)
        min_index = numdifference.index(min_value)
        return possible[min_index]
...
def feline_flips(start, goal, limit):
    ...
    if start == goal:
        ...
        goal_list = list(goal)
        if start_list == [] or goal_list == []:
            return abs(len(start_list) - len(goal_list))
        else:
            if start_list[0] != goal_list[0]:
                del start_list[0]
                del goal_list[0]
                return 1 + feline_flips(start_list, goal_list, limit - 1)
            else:
                del start_list[0]
                del goal_list[0]
                return 0 + feline_flips(start_list, goal_list, limit)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    if start == goal:  # Fill in the condition
        ...
        substitute = difference + minimum_mewtations(start[1:], goal[1:], limit - difference)
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def time_per_word(words, times_per_player):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    new_list = match([words[i] for i in range(len(words))], [[times_per_player[j][i + 1] - times_per_player[j][i] for i in range(len(times_per_player[j]) - 1)] for j in range(len(times_per_player))])
    return new_list
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for b in range(len(get_times(match)[0])):
        count = 0
        smallest = min([get_times(match)[a][b] for a in player_indices])
        for c in player_indices:
            if smallest == get_times(match)[c][b]:
                count = count + 1
                if count == 1:
                    list[c].append(word_at(match, b))
    return list
    # END PROBLEM 10
