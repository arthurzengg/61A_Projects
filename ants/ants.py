...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        my_place = self.place
        count = 0
        while not my_place.is_hive:
            if my_place.bees and self.min_range <= count <= self.max_range:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # for bee in my_place.bees[:]:
        #         bee.reduce_health(amount)
        # if self.health <= 0:
        #     for bee in my_place.bees[:]:
        #         bee.reduce_health(damage)
        for bee in self.place.bees[:]:
            bee.reduce_health(amount + self.damage if self.health <= amount else amount)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.chew_countdown > 0:
            ...
            my_place = self.place
            bee_eated = random_bee(my_place.bees)
            if bee_eated:
                bee_eated.reduce_health(bee_eated.health)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not gamestate.if_queen_ant:
            gamestate.if_queen_ant = True
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        first = self.place.exit
        while first.exit:
            if first.ant:
                first.ant.buff()
                if first.ant.is_container and first.ant.ant_contained:
                    first.ant.ant_contained.buff()
            first = first.exit
    ...
    def action(self, gamestate):
        ...
        if self.slow_time:
            if gamestate.time % 2 == 1:
                if_move = False
            self.slow_time -= 1
        if self.scare_time and if_move:
